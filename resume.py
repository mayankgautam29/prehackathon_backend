...
# FIXED REGEX – detects ALL valid GitHub URLs
...
async def validate_links(text: str):
    ...
    results = []
    for link in links:
        is_valid = await check_github_link(link)
        results.append({
            "link": link,
            "status": "Valid" if is_valid else "Invalid or Private"
        })
    return results
